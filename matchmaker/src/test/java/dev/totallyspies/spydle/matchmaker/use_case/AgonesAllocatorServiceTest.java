...
public class AgonesAllocatorServiceTest {
  ...
  @Mock private AllocationServiceGrpc.AllocationServiceStub allocationServiceStub;
  ...
  public void setUp() {
    ...
    agonesAllocatorService =
        new AgonesAllocatorService(
            gameFleet, gameNamespace, gameServerRepository, allocationServiceStub);
  }

  @Test
  public void testAwaitAllocation_Success() {
    Allocation.AllocationResponse response =
        Allocation.AllocationResponse.newBuilder()
            .setGameServerName("game-server-12345")
            .setAddress("127.0.0.1")
            .addPorts(
                Allocation.AllocationResponse.GameServerStatusPort.newBuilder()
                    .setPort(7777)
                    .build())
            .build();
    ...
    doAnswer(
        ...
        .when(allocationServiceStub)
        .allocate(any(), any());

    GameServer gameServer =
        new GameServer("127.0.0.1", 7777, "", "", true, GameServer.State.WAITING);
  ...
  public void testAwaitAllocation_GameServerNotExist() {
    Allocation.AllocationResponse response =
        Allocation.AllocationResponse.newBuilder().setGameServerName("game-server-12345").build();
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> agonesAllocatorService.awaitAllocation(1));
