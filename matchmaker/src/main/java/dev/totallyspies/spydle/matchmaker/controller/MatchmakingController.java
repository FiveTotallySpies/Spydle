...
/**
 * Responsible for delegating requests to client-facing endpoints. create-game: Allocate a game
 * server to the user and assign them a session for it join-game: Assign a session for a user to an
 * already allocated existing game server leave-game: Delete a user session from a game server and
 * notify it of client departure
 */
@RestController
@Validated
public class MatchmakingController {
  ...
  public ResponseEntity<?> createGame(@Valid @RequestBody CreateGameRequestModel request) {
    ...
    if (clientId == null) {
      ...
      return ResponseEntity.status(400)
          .body(new ClientErrorResponse().message("Bad clientId: should be UUID"));
    }
    try {
      GameServer gameServer =
          matchmakingService.createGame(clientId, request.getPlayerName(), 5000);
      logger.info("Successfully handled /create-game request: {}", gameServer);
      return ResponseEntity.ok(
          new CreateGameResponseModel()
              .gameServer(gameServer)
  ...
  public ResponseEntity<?> joinGame(@Valid @RequestBody JoinGameRequestModel request) {
    UUID clientId = sessionRepository.parseClientId(request.getClientId());
    logger.info(
        "Received request: /join-game, clientId: {}, gameServerName: {}",
        request.getClientId(),
        request.getRoomCode());
    if (clientId == null) {
    ...
    try {
      if (!gameServerRepository.gameServerExists(request.getRoomCode())) {
        logger.info(
            "Request for /join-game requests room that doesn't exist: {}", request.getRoomCode());
        return ResponseEntity.status(404)
            .body(
                new ClientErrorResponse()
                    .message("Could not find game server: " + request.getRoomCode()));
      }
      if (sessionRepository.sessionExists(clientId)) {
        logger.info(
            "Request for /join-game is try to join room {} when they are already in {}",
            request.getRoomCode(),
            sessionRepository.getSession(clientId).getGameServer().getRoomCode());
        return ResponseEntity.status(403)
            .body(new ClientErrorResponse().message("Cannot join room: you are already in one"));
      }
