...
public class MatchmakingService {
  ...
  public GameServer createGame(UUID clientId, String playerName, int timeoutMillis) {
    ...
    ClientSession session =
        new ClientSession(clientId, allocated, playerName, ClientSession.State.ASSIGNED);
  ...
  public void joinGame(UUID clientId, String playerName, GameServer room) {
    ...
    sessionRepository.saveSession(
        new ClientSession(clientId, room, playerName, ClientSession.State.ASSIGNED));
  }

  public List<String> listGames() {
    return gameServerRepository.getGameServers().stream()
        .filter(
            gameServer ->
                gameServer.isPublicRoom() && gameServer.getState() == GameServer.State.WAITING)
