...
public class GameRoomPresenter {
    ...
    for (Player player : updatePlayerList.getPlayersList()) {
      model
          ...
          .put(
              ...
              new GameRoomViewModel.Guess("", GameRoomViewModel.Guess.Verdict.IN_PROGRESS));
    }
    view.updateGame();
  }
  ...
  public void onGuessUpdateMessage(CbGuessUpdate guessUpdate) {
    ...
    GameRoomViewModel.Guess guess =
        model.getCurrentGuesses().get(guessUpdate.getPlayer().getPlayerName());
  ...
  public void onGuessResultMessage(CbGuessResult guessResult) {
    GameRoomViewModel.Guess guess =
        model.getCurrentGuesses().get(guessResult.getPlayer().getPlayerName());
    guess.setCurrentWord(guessResult.getGuess());
    guess.setVerdict(
        guessResult.getCorrect()
            ? GameRoomViewModel.Guess.Verdict.CORRECT
            : GameRoomViewModel.Guess.Verdict.INCORRECT);
    view.updateGuessProgress();
  }
}
