...
public class CreateGameInteractor implements CreateGameInputBoundary {
  ...
  public CreateGameInteractor(
      @Value("${server.backend.gameserver-overwrite}") String gameServerOverwrite,
  ...
  public CreateGameOutputData execute(CreateGameInputData data) {
    ...
    CreateGameRequestModel request =
        new CreateGameRequestModel().clientId(clientId.toString()).playerName(data.getPlayerName());
    Object response =
        webClient.postEndpoint("/create-game", request, CreateGameResponseModel.class);
    if (response instanceof CreateGameResponseModel responseModel) {
      String ip =
          gameServerOverwrite == null || gameServerOverwrite.isBlank()
