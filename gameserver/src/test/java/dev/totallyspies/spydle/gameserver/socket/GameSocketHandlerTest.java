...
public class GameSocketHandlerTest {
  ...
  @Mock private ClientSessionValidator sessionValidator;
  @Mock private SbMessageListenerProcessor annotationProcessor;
  ...
  public void testHandleBinaryMessage_ValidMessage() throws Exception {
    ...
    headers.put(
        SharedConstants.CLIENT_ID_HTTP_HEADER, Collections.singletonList(clientId.toString()));
    headers.put(SharedConstants.CLIENT_NAME_HTTP_HEADER, Collections.singletonList(clientName));
    when(socketSession.getHandshakeHeaders())
        .thenReturn(new org.springframework.http.HttpHeaders(headers));
  ...
  public void testHandleBinaryMessage_InvalidClientSession() throws Exception {
    ...
    verify(socketSession)
        .close(argThat(status -> status.getCode() == CloseStatus.NOT_ACCEPTABLE.getCode()));
  ...
  public void testHandleBinaryMessage_ExceptionDuringParsing() throws Exception {
    ...
    try (MockedStatic<SbMessage> sbMessageMockedStatic = Mockito.mockStatic(SbMessage.class)) {
      sbMessageMockedStatic
          .when(() -> SbMessage.parseFrom(payload))
  ...
  public void testAfterConnectionEstablished_ClientNameAlreadyExists() throws Exception {
    ...
    ClientSession existingSession =
        fakeClientSession.toBuilder().clientId(existingClientId).playerName(clientName).build();
    ...
    MultiValueMap<String, String> headers = new LinkedMultiValueMap<>();
    headers.put(
        SharedConstants.CLIENT_ID_HTTP_HEADER, Collections.singletonList(newClientId.toString()));
    headers.put(
        SharedConstants.CLIENT_NAME_HTTP_HEADER,
        Collections.singletonList(clientName)); // Same name
    when(newSocketSession.getHandshakeHeaders())
        .thenReturn(new org.springframework.http.HttpHeaders(headers));
  ...
  public void testAfterConnectionClosed_NormalClosure() throws Exception {
    ...
    // Mock stored session
    ClientSession storedSession =
        fakeClientSession.toBuilder().clientId(clientId).playerName(clientName).build();
  ...
  public void testCloseAllSessions() throws Exception {
    ClientSession clientSession1 =
        fakeClientSession.toBuilder().clientId(UUID.randomUUID()).build();
