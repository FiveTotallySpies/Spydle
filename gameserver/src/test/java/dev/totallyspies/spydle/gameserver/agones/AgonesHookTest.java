...
public class AgonesHookTest {
    ...
    @Mock
    private CompletableFuture<Sdk.GameServer> gameServerFuture;

    @Mock
    private Sdk.GameServer sdkGameServer;
    ...
    public void testConstructor_Success() throws ExecutionException, InterruptedException {
        ...
        Sdk.GameServer.ObjectMeta objectMeta = Sdk.GameServer.ObjectMeta.newBuilder()
                .setName(gameServerName)
                .build();
        when(sdkGameServer.getObjectMeta()).thenReturn(objectMeta);

        Sdk.GameServer.Status.Port port = Sdk.GameServer.Status.Port.newBuilder()
                .setPort(expectedPort)
                .build();
        Sdk.GameServer.Status status = Sdk.GameServer.Status.newBuilder()
        ...
        // Since it's a simple static utility method, we can use the real implementation or assume the behavior
    ...
    @Test
    public void testConstructor_InterruptedException() throws ExecutionException, InterruptedException {
        // Arrange
        when(agones.getGameServerFuture()).thenReturn(gameServerFuture);
