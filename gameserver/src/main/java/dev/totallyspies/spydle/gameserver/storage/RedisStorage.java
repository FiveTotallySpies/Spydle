...
/**
 * Wrapper for an external redis storage database that is likely shared between this gameserver and
 * other gameservers.
 */
@Service
@Primary
...
public class RedisStorage implements GameServerStorage {
  ...
  public RedisStorage(RedisTemplate<String, Object> redisTemplate) {
    ...
    LoggerFactory.getLogger(RedisStorage.class)
        .info("Found storage.type=redis, loading RedisStorage");
  }
  ...
  public ClientSession getClientSession(UUID clientId) {
    ...
    if (!(value instanceof ClientSession session)) {
      throw new IllegalStateException(
          "Redis storage for client session "
              + clientId
              + " has invalid type "
              + value.getClass().getCanonicalName());
    }
